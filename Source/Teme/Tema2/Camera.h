...
public:
	...
	void RotateOX(float angle, glm::vec3 targetPosition)
	{
		if (type == FirstPerson)
			RotateFirstPerson_OX(angle);
		else {
			ComputeDistanceToTarget(targetPosition);
			RotateThirdPerson_OX(angle);
		}
	}

	void RotateOY(float angle, glm::vec3 targetPosition)
	{
		if (type == FirstPerson)
	...
	void ComputeDistanceToTarget(glm::vec3 targetPos) {
		distanceToTarget =  glm::distance(position, targetPos);
	}

public:
	int type = FirstPerson;
